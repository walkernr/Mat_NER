...
class CRF(nn.Module):
    ...
    def forward(self, fc_out, tags):
        ...
        if tags.size()[:2] != fc_out.size()[:2]:
            tags = torch.cat((tags, torch.zeros(tags.size()[0], 1, dtype=torch.long, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))), axis=1)
