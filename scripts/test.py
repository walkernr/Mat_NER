...
from model_ner import BiLSTM_NER, Transformer_NER, BERT_NER
...
for alias, split in splits.items():
    ...
    corpus = DataCorpus(data_path=data_path, data_name=data_name, alias=alias, vector_path=vector_path,
                        tokenizer=tokenizer, max_sequence_length=None,
                        cased=cased, bert=False, batch_size=batch_size, device=device)
    ...
    bilstm_trainer = NERTrainer(model=bilstm, data=corpus, optimizer_cls=Adam, criterion_cls=nn.CrossEntropyLoss, full_finetuning=full_finetuning, max_grad_norm=max_grad_norm, device=device)
    # initialize trainer class for transformer
    transformer_trainer = NERTrainer(model=transformer, data=corpus, optimizer_cls=Adam, criterion_cls=nn.CrossEntropyLoss,  full_finetuning=full_finetuning, max_grad_norm=max_grad_norm, device=device)
    ...
    for i, q in enumerate(quant):
        for j, p in enumerate(phase):
            ...
            if j == 1 and i == 0:
                ax.set_title(phase[j])
    # save figure
    fig.savefig('history{}.png'.format(alias))
    plt.close()